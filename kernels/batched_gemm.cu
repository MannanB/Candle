...
void launch_batched_mat_mul_kernel(float* A, float* B, float* C, int batch_dim, int A_rows, int shared_dim, int B_cols) {
    ...
    dim3 threadsPerBlock(THREADS_PER_BLOCK_MM, THREADS_PER_BLOCK_MM, THREADS_PER_BATCH);
    dim3 numBlocks(cuda::ceil_div(B_cols, TILE_WIDTH), cuda::ceil_div(A_rows, TILE_WIDTH), cuda::ceil_div(batch_dim, BATCH_TILE_WIDTH),);

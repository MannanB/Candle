...
__global__ void batched_mat_mul_kernel(float* A, float* B, float* C, int batch_dim, int A_rows, int shared_dim, int B_cols) {
    // C -> A_rows x B_cols

    const int batch = blockIdx.z; // TODO: tile_width vs threads_per_block
    ...
    for (int tile = 0; tile < num_tiles; tile++) {
        ...
        if (batch < batch_dim && row < A_rows && (tile * blockDim.x + threadIdx.x) < shared_dim) {
            tile_shared_mem_A[threadIdx.y][threadIdx.x] = A[batch * A_rows * shared_dim + row * shared_dim + (tile * blockDim.x + threadIdx.x)];
        } else {
    ...
    if (row < A_rows && col < B_cols) {
        C[batch * A_rows * B_cols + row*B_cols + col] = value;
    }
}

void launch_batched_mat_mul_kernel(float* A, float* B, float* C, int batch_dim, int A_rows, int shared_dim, int B_cols) {

    dim3 threadsPerBlock(THREADS_PER_BLOCK_MM, THREADS_PER_BLOCK_MM);
    dim3 numBlocks(cuda::ceil_div(B_cols, TILE_WIDTH), cuda::ceil_div(A_rows, TILE_WIDTH), batch_dim);


    batched_mat_mul_kernel<<<numBlocks, threadsPerBlock>>>(A, B, C, batch_dim, A_rows, shared_dim, B_cols);

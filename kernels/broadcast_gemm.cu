#include "broadcast_gemm.h"

#define THREADS_PER_BLOCK_MM 16
#define TILE_WIDTH 16


__global__ void broadcast_mat_mul_kernel(float* A, float* B, float* C, int batched_dim, int A_rows, int shared_dim, int B_cols) {
    // C -> A_rows x B_cols
...
void launch_broadcast_mat_mul_kernel(float* A, float* B, float* C, int batched_dim, int A_rows, int shared_dim, int B_cols) {
    // A is m x k, B is batched_dim x k x n (n=1 for NNs)
    ...
    broadcast_mat_mul_kernel<<<numBlocks, threadsPerBlock>>>(A, B, C, batched_dim, A_rows, shared_dim, B_cols);
